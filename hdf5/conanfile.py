...
class Hdf5Conan(ConanFile):
    ...
    build_requires = "cmake/[>=3.12.0]@scine/stable"
    _cmake = None
    ...
    def source(self):
        tools.get("https://github.com/live-clones/hdf5/archive/hdf5-1_12_0.tar.gz")
        extracted_dir = "{0}-{0}-{1}".format(self.name,
    ...
    def _patch_sources(self):
        ...
        tools.patch(base_path=self._source_subfolder,
                    patch_file="patches/config.patch")
        tools.patch(base_path=self._source_subfolder,
                    patch_file="patches/filters.patch")
        # Do not force PIC
    ...
    def _configure_cmake(self):
        ...
        self._cmake.definitions["HDF5_EXTERNALLY_CONFIGURED"] = False
        self._cmake.definitions["HDF5_EXTERNAL_LIB_PREFIX"] = ""
        self._cmake.definitions["HDF5_USE_FOLDERS"] = False
        self._cmake.definitions["HDF5_NO_PACKAGES"] = False
        ...
        if self.settings.build_type == "Debug":
            # Option?
            self._cmake.definitions["HDF5_ENABLE_INSTRUMENT"] = False
        ...
        self._cmake.definitions["HDF5_ENABLE_SZIP_SUPPORT"] = bool(
            self.options.szip_support)
        if bool(self.options.szip_support):
            # this variable is added by conanize-link-szip*.patch
            self._cmake.definitions["CONAN_SZIP_LIBNAME"] = self._get_szip_lib(
            )
        self._cmake.definitions["HDF5_ENABLE_SZIP_ENCODING"] = self.options.get_safe(
            "szip_encoding") or False
        self._cmake.definitions["HDF5_PACKAGE_EXTLIBS"] = False
        self._cmake.definitions["HDF5_ENABLE_THREADSAFE"] = self.options.get_safe(
            "threadsafe") or False
        self._cmake.definitions["HDF5_ENABLE_DEBUG_APIS"] = False  # Option?
        self._cmake.definitions["BUILD_TESTING"] = False
        self._cmake.definitions["HDF5_INSTALL_INCLUDE_DIR"] = os.path.join(
            self.package_folder, "include", "hdf5")
